...
function handleWifiStatus(req, res) {
  try {
    ...
    // Client mode — query the OS for current association + signal. iwgetid
    // can return empty on modern Pi OS (Trixie) + NetworkManager even when
    // wlan0 *is* associated, so fall back to nmcli when it does.
    ...
    if (signal === null) {
      // nmcli reports 0-100 quality; same conversion as the scan path.
      try {
        const q = execSync(
        ...
        const n = parseInt(q);
        if (!isNaN(n) && n > 0) signal = Math.round((n / 2) - 100);
      } catch (_) {}
    }
...
// Drop a NetworkManager conf.d snippet disabling WiFi power management for
// every connection. The single biggest source of "spotty WiFi" on Pi 3B+ is
// the brcmfmac driver's aggressive radio-idle: every few seconds the chip
// powers down, drops pings, NM marks the link down, then reassociates a few
// seconds later. With wifi.powersave=2 the radio stays hot. Cost: ~50mW.
// Idempotent — runs on every startup, only writes the file if missing.
function disableWifiPowersave() {
  try {
    const NM_CONF_DIR = '/etc/NetworkManager/conf.d';
    const conf = `${NM_CONF_DIR}/displayboard-wifi-powersave.conf`;
    let exists = false;
    try { execSync(`sudo test -f "${conf}"`, { timeout: 2000 }); exists = true; } catch (_) {}
    if (!exists) {
      const content = '# Installed by DisplayBoard — keep the WiFi radio hot to avoid\n' +
                      '# brcmfmac power-save drops on Pi 3B+/4. 2 = disabled.\n' +
                      '[connection]\nwifi.powersave = 2\n';
      const tmp = '/tmp/displayboard-wifi-powersave.conf';
      fs.writeFileSync(tmp, content);
      execSync(`sudo mkdir -p "${NM_CONF_DIR}"`, { timeout: 2000 });
      execSync(`sudo cp "${tmp}" "${conf}"`, { timeout: 2000 });
      execSync(`sudo chmod 644 "${conf}"`, { timeout: 2000 });
      try { fs.unlinkSync(tmp); } catch (_) {}
      try { execSync('sudo systemctl reload NetworkManager', { timeout: 5000 }); } catch (_) {}
      console.log('📶 Installed wifi.powersave=2 NetworkManager drop-in');
    }
    // NM reloads apply to *new* connections; the current wlan0 association
    // may not flip until reconnect. Set it on the running interface too so
    // the fix takes effect without rebooting.
    try { execSync('sudo iw dev wlan0 set power_save off 2>/dev/null', { timeout: 2000 }); } catch (_) {}
  } catch (e) {
    console.error('WiFi powersave config skipped:', e.message);
  }
}
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  // Single biggest WiFi-resilience win on Pi 3B+/4: disable the radio's
  // aggressive power-save. Brcmfmac without this drops pings every few seconds.
  setTimeout(disableWifiPowersave, 6500);
